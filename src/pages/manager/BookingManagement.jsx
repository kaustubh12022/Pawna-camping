...
const BookingManagement = () => {
    ...
    return (
        <div className="space-y-6">
            ...
            {/* TABLE AND CARDS */}
            <div className="bg-white md:rounded-2xl md:border border-stone-100 overflow-hidden">
                {filtered.length === 0 ? (
                    ...
                    <>
                        ...
                        <div className="md:hidden flex flex-col gap-4 py-2">
                            {filtered.map(b => (
                                <div key={b._id} className="bg-white border border-stone-200 rounded-2xl p-4 shadow-sm flex flex-col gap-3">
                                    <div className="flex justify-between items-start border-b border-stone-100 pb-3">
                                        <div>
                                            <div className="font-bold text-stone-900">{b.customerName}</div>
                                            <div className="text-sm text-stone-500 mt-0.5">{b.customerPhone}</div>
                                        </div>
                                        <div className="text-right">
                                            <div className="text-sm font-bold text-stone-800">{b.property?.name || '—'}</div>
                                            <div className="text-[11px] text-stone-500 uppercase tracking-wider font-semibold mt-0.5">{b.packageType || b.propertyType}</div>
                                        </div>
                                    </div>
                                    
                                    <div className="flex justify-between items-center bg-stone-50 rounded-xl p-3 text-sm">
                                        ...
                                        <div className="font-bold text-emerald-700">{b.guests} Guests</div>
                                    </div>
                                    ...
                                    <div className="flex justify-end gap-2 pt-2">
                                        ...
                                        {b.status !== 'cancelled' && (
                                            <button onClick={() => handleUpdateStatus(b._id, 'cancelled')}
                                                className={`${b.status === 'pending' ? 'flex-1' : 'w-full'} inline-flex justify-center items-center gap-1.5 px-4 py-2.5 bg-stone-100 text-stone-700 text-sm font-bold rounded-xl hover:bg-stone-200 transition-colors active:scale-[0.98]`}>
                                                <XIcon className="w-4 h-4" /> Cancel
                                            </button>
                                        )}
                                        <button onClick={() => handleDelete(b._id)}
                                            className="px-4 py-2.5 rounded-xl border border-stone-200 hover:bg-red-50 text-stone-400 hover:text-red-600 hover:border-red-200 transition-colors active:scale-[0.98]">
                                            <Trash2 className="w-4 h-4" />
                                        </button>
                                    </div>
                                </div>
